...
%%% Parametros
N_q=6; omega=1:-1e-2:-1.6; la=0.25; wn= 2 ; zi=0.3; 
...
b= (omegat==-1.2); [n,m]=max(b); solucion=qn(m,:); 
...
%%% HAMILTON
DI= 0.5*[(Qnv(:,1).^2 + la*Qnv(:,1).*Qnv(:,2)) , Qnv(:,2:N_q-1).^2 + la*Qnv(:,2:N_q-1).*( Qnv(:,1:N_q-2) + Qnv(:,3:N_q) ), Qnv(:,N_q).^2 + la*Qnv(:,N_q).*Qnv(:,N_q-1)]; 
...
figure(2); plot(solucion); xlabel('Resonador','LineWidth',1.2); ylabel('Amplitud (qn)','LineWidth',1.2) ; title('Solucion estacionaria a propagar','LineWidth',1.7)

figure(3); imagesc(qn); xlabel('Resonador','LineWidth',1.2); ylabel('Omega','LineWidth',1.2) ; title('Soluciones estacionarias','LineWidth',1.2)

figure(4); plot(t,Qn);  xlabel('Tiempo'); ylabel('Amplitud (Qn)') ; title('Qn vs t')
figure(10); plot(t,Qnv);  xlabel('Tiempo'); ylabel('Amplitud (Qn)') ; title('Qn vs t')
